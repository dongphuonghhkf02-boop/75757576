...
## История изменений
### 2026-05-16 — `/product`: describeSection / Image2 переписан pixel-perfect по Figma SVG-эталону
Пользователь предоставил Figma-экспорт в виде SVG (1920×1395) — извлёк точные координаты всех элементов через regex-парсинг path-данных:

| Элемент | Координаты (design px) | Цвет |
|---|---|---|
| describeSection canvas | 1920 × 1395 | bg white |
| Tree image | x=536 y=424 w=1602 h=828 | object-fit contain |
| Chip 1 «Швидке відновлення» | x=1417 y=576 w=382 h=120 | bg `rgba(255,255,255,0.2)` + text #F9F7F2 |
| Chip 2 «Ідеальний pH-баланс» | x=790 y=812 w=382 h=120 | bg `rgba(255,255,255,0.2)` + text #F9F7F2 |
| Chip 3 «Покращення поглинання» | x=1417 y=1064 w=382 h=120 | bg `rgba(255,255,255,0.2)` + text #2C2C27 |
| White fade overlay (top→bottom) | x=97 y=597 w=1920 h=684 | linear-gradient(transparent 67.5% → white) |
| Tag «Проблема» | x=144 y=544 w=92 h=22 | bg #E7EBE7 |
| Tag «Рішення» | x=144 y=951 w=82 h=22 | bg #F7FAE8 |

**Главное открытие**: все 3 chip-overlays используют ОДИН тот же фон `rgba(255,255,255,0.2)` (стеклянное стекло), отличаются только цветом текста и иконок. Прошлая итерация неверно использовала разные оливковые/тёмные градиенты.

**Фиксы**:
- `image2.module.css`: переписан полностью под SVG-coords (absolute `top/left` в design-px)
- Все позиции элементов теперь относительные к describeSection top-left (0,0)
- Backdrop-filter `blur(10px)` на чипах для glass-морфизма
- Chip 1+2: `filter: brightness(0) invert(1)` для cream-иконок
- Chip 3: `filter: brightness(0)` для чёрной иконки
- Image source: `image (3).png` от пользователя → перезаписал `/app/frontend/public/tree.png`
- Реальный размер describeSection: **1920×1401** vs дизайн 1920×1395 (Δ=6px, в пределах допуска)

### 2026-05-16 — `/product`: блок «Відновлення після стресу. Стабільний врожай» приведён к Figma
- **Главная проблема**: вместо дерева с прозрачным фоном стояла фотография поля (/image@2x.png), компонент Image2 был спозиционирован абсолютно с `right: -218px` (вылезал за пределы секции), 3 overlay-чипа использовали миксовые градиенты.
- **Фиксы** в `image2.module.css` + `image2.tsx`:
  - src: `/image@2x.png` (1602×828, поле) → **`/tree.png`** (1376×601, дерево с прозрачным фоном)
  - `.image` позиционирование: `right: -218 bottom: 114` → **`top: 217 right: 0`** (внутри describeSection)
  - Удалён нижний градиент-overlay, добавлен левый soft-fade overlay для плавного слияния с белой картой
  - Чипы переведены на glass-эффект с `backdrop-filter: blur(8px)`:
    - Chip 1: `rgba(179, 210, 23, 0.55)` (brand-accent-primary @ 55%) + белый текст + иконка молнии (Vector14.svg)
    - Chip 2: `rgba(44, 44, 39, 0.65)` (text-black @ 65%) + белый текст + Drop+hand (Vector15.svg)
    - Chip 3: `rgba(255, 255, 255, 0.85)` + чёрный текст + Drop (Vector6.svg)
  - Chip каптоны 14/400/100% + body 18/600/120% — точно по Figma токенам
- Все позиции overlay-чипов теперь визуально совпадают с эталоном Figma.
...
| Параметр | Было | Стало (Figma) |
|---|---|---|
| Размер | 1920×579 (auto) | **1920 × 564** |
| Padding | 120 со всех сторон | **120 top/left/right + 32 bottom** |
| Logo-track | 480px | **776px** (асет 366px, gap до col1 = 424) |
| Col1 «Нашим клієнтам» — X | 600 | **896** |
| Col2 «Наша адреса» — X | 1000 | **1190** |
| Col3 «Контакти» — X | 1400 | **1519** |
| Grid template | `480 1fr 1fr 1fr` | **`776 294 329 1fr`** |
| Сoциалки → credits | 60 | 60 |
| Credits row Y | ~525 | ~520 |

Ключевая ошибка прошлой итерации: вся раскладка предполагала `padding: 120` со всех сторон, но реальный Figma имеет padding-bottom всего ~32px, и три инфо-колонки сгруппированы в правой части футера (Х = 896/1190/1519), а не равномерно распределены. Это была причина «слишком большого отступа снизу» и «клиенты слишком близко к логотипу».

### 2026-05-16 — Footer (`/`) приведён к Figma (итерация 1)
- `.footer.height: 576 → auto (min-height: 579)`
